...
fopen_path(const char *path, const char *mode) {
    ...
    /*Debug check*/
    if(fp == NULL)
         {   LOGE("Can mount but Can't open  %s\n", path); }
    return fp;
}
...
main(int argc, char **argv) {
    ...
    if (update_package) {
        ...
        //FR-550496, Add by changmei.chen@tcl.com for JrdFota upload error code to GOTU server , 2013-11-18, begin
        // JRD
        /* below code is nothing useful now, just omit them temply
        if (strncmp(update_package, "/mnt/shell/emulated/0", 21) == 0) {
            int len = strlen(update_package) + 20;
            char* modified_path = (char*)malloc(len);
            memset(modified_path, 0, len);
            strlcpy(modified_path, "/data/media/0", len);
            strlcat(modified_path, update_package + 21, len - 21 + 1);
            fprintf(stdout, "(replacing path \"%s\" with \"%s\")\n",
                   update_package, modified_path);
            update_package = modified_path;
            ensure_path_mounted("/data");
        } else if (strncmp(update_package, "/storage/emulated/legacy", 24) == 0) {
            int len = strlen(update_package) + 20;
            char* modified_path = (char*)malloc(len);
            memset(modified_path, 0, len);
            strlcpy(modified_path, "/data/media/0", len);
            strlcat(modified_path, update_package + 24, len - 24 + 1);
            fprintf(stdout, "(replacing path \"%s\" with \"%s\")\n",
                   update_package, modified_path);
            update_package = modified_path;
            ensure_path_mounted("/data");
        } else if (strncmp(update_package, "/storage/emulated/0", 19) == 0) {
            int len = strlen(update_package) + 20;
            char* modified_path = (char*)malloc(len);
            memset(modified_path, 0, len);
            strlcpy(modified_path, "/data/media/0", len);
            strlcat(modified_path, update_package + 19, len - 19 + 1);
            fprintf(stdout, "(replacing path \"%s\" with \"%s\")\n",
                   update_package, modified_path);
            update_package = modified_path;
            ensure_path_mounted("/data");
        ...
        */
         ...
         *end
         */
        /*FOTA internal sdcard is /storage/sdcard0, needn't transfer
